...
*A puzzle game for getting comfortable with git by pruning bonsai trees. UI is Japanese-only.*
...
3. お題どおりの樹形に整えばクリア。見本は「答えを見る」でいつでも確認でき、クリア時には実際に打つはずだった git コマンドが答えとして表示される

成立しない操作は理由つきで弾かれるので、なぜダメだったかがその場で分かります。迷ったらヒントを——まず考え方だけが出て、操作まで明かすかは自分で選べます。20 問はステージ制で、進んだ分だけホームの盆栽に花が咲きます。

キーボードでも一通り遊べます: Z = 一手戻す / R = 最初から / Shift + ← → = 問の移動 / Esc = 選択解除。
